...
contract Depositor is CCIPBase {
    ...
    function setMainRouterChainSelector(
        uint64 _newMainRouterChainSelector
    ) external onlyOwner {
    ...
    function depositAndMint(
        ...
        if (chainSelector == mainRouterChainSelector) {
            emit Deposit(msg.sender, _token, _amount);
            IMainRouter(mainRouter).depositAndMint(
                msg.sender,
                chainSelector,
                _token,
                _amount,
        ...
        bytes memory _data = abi.encode(
            TransactionReceive.DEPOSIT_MINT,
            abi.encode(
                msg.sender,
                _token,
                _amount,
    ...
    function redeem(
        ...
        address _token,
        uint256 _amount
    ) external onlyMainRouter(msg.sender) {
    ...
    {
        (TransactionSend _transactionType, bytes memory _data) = abi.decode(
            message.data,
            (TransactionSend, bytes)
        );
        if (_transactionType == TransactionSend.REDEEM) {
            (address _from, address _to, address _token, uint256 _amount) = abi
                .decode(_data, (address, address, address, uint256));
            _redeem(_from, _to, _token, _amount);
