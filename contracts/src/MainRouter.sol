...
contract MainRouter is CCIPBase, FunctionsBase {
    ...
    address private optimismDepositor;
    address private optimismMinter;
    ...
    modifier onlyOptimismDepositor(address _caller) {
        if (optimismDepositor != _caller) {
            revert NotAllowed();
        }
        _;
    }

    modifier onlyOptimismMinter(address _caller) {
        if (optimismMinter != _caller) {
            revert NotAllowed();
        }
        _;
    }
    ...
    function redeem(
        ...
        address _token,
        uint256 _amount
    ) external payable onlyAllowedToken(_destinationChainSelector, _token) {
        if (_amount == 0) {
    ...
    ) external onlyOptimismDepositor(msg.sender) {
    ...
    function burnAndMint(
        ...
        uint64 _destinationChainSelector,
        address _receiver
    ) external onlyOptimismMinter(msg.sender) {
    ...
    function _liquidate(
        ...
        uint256 _usdToRedeem = _amountToCover +
            (_amountToCover * LIQUIDATION_PENALTY) /
            LIQUIDATION_PRECISION;
        uint256 _amount = min(
            deposited[_liquidatedUser][_destinationChainSelector][_token],
            convertUSDIntoToken(_usdToRedeem, _destinationChainSelector, _token)
        );
        _redeem(
            _destinationChainSelector,
            _receiver,
    ...
    function _redeem(
        ...
        bytes memory _data = abi.encode(
            TransactionSend.REDEEM,
            abi.encode(_from, _to, _token, _amount)
        );
        _ccipSend(
            _to,
            _destinationChainSelector,
    ...
    function _mint(
        ...
        bytes memory _data = abi.encode(
            TransactionSend.MINT,
            abi.encode(_sender, _amount)
        );
        _ccipSend(
            _sender,
            _destinationChainSelector,
    ...
    function getRedeemFee(
        ...
        bytes memory _data = abi.encode(
            TransactionSend.REDEEM,
            abi.encode(msg.sender, _token, _amount)
        );
    ...
    function getUserFractionToLTV(
        ...
        ) = getUserOverallInformation(_user);
        fraction = calculateUserFractionToLTV(
            _totalCollateral,
            _totalMinted,
            _userLTV
        );
    ...
    function calculateUserFractionToLTV(
        uint256 _totalCollateral,
        uint256 _totalMinted,
        uint256 _ratio
    ...
    function calculateFraction(
        ...
        uint256 _totalMinted,
        uint256 _ratio
    ) public pure returns (uint256 answer) {
    ...
    function calculateLTV(address _user) public view returns (uint256) {
        return
            BASE_LTV +
            convertCreditToLTV(
                userActivityCredit[_user] - userProtocolCredit[_user]
            );
    }

    function convertCreditToLTV(
        uint256 _activityCredit
    ...
    function getUserOverallInformation(
        address user
    ) public view returns (uint256 totalCollateral, uint256 totalMinted) {
    ...
    function getUserCollateralOnChainValue(
        ...
        EnumerableSet.AddressSet storage _chainAllowedTokens = allowedTokens[
            _chainSelector
        ];
        ...
        for (uint i = 0; i < _tokenLength; i++) {
            ...
            totalAmount += getUserCollateralValue(
                _user,
                _chainSelector,
                _token
            );
    ...
    function getUserCollateralValue(
        ...
        return
            getCollateralValue(
                _chainSelector,
                _token,
                getUserDepositedAmount(_user, _chainSelector, _token)
            );
    }
    ...
    function getCollateralValue(
        ...
        address _token,
        uint256 _amount
    ) public view onlyAllowedToken(_chainSelector, _token) returns (uint256) {
        AggregatorV3Interface _priceFeed = AggregatorV3Interface(
            priceFeeds[_chainSelector][_token]
        );
        (, int256 _price, , , ) = _priceFeed.staleCheckLatestRoundData();
        return
            ((uint256(_price) * FEED_PRECISION) *
                _amount *
    ...
    function getAllowedTokensOnChain(
        ...
        address[] memory _tokens = new address[](
            allowedTokens[_chainSelector].length()
        );
    ...
    {
        (TransactionReceive _transactionType, bytes memory _data) = abi.decode(
            message.data,
            (TransactionReceive, bytes)
        );
        uint64 _sourceChainSelector = message.sourceChainSelector;
        if (_transactionType == TransactionReceive.DEPOSIT) {
            (address _depositor, address _token, uint256 _amount) = abi.decode(
                _data,
                (address, address, uint256)
            );
            _deposit(_depositor, _sourceChainSelector, _token, _amount);
        } else if (_transactionType == TransactionReceive.BURN) {
            (address _burner, uint256 _amount) = abi.decode(
                _data,
                (address, uint256)
            );
            _burn(_burner, _sourceChainSelector, _amount);
            ...
            (
                ...
                uint256 _amountToMint
                    ...
                    _data,
                    (address, address, uint256, uint64, address, uint256)
                );
            _deposit(_depositor, _sourceChainSelector, _token, _amount);
            _mint(
                _destinationChainSelector,
            ...
            (
                ...
                uint64 _destinationChainSelector,
                address _receiver
            ) = abi.decode(_data, (address, uint256, uint64, address));
            _burn(_burner, _sourceChainSelector, _amount);
            _mint(_destinationChainSelector, _receiver, _burner, _amount);
            ...
            (
                ...
                address _sender
                    ...
                    _data,
                    (address, address, uint64, address, uint256, address)
                );
            _liquidate(
                _sourceChainSelector,
                _liquidatedUser,
