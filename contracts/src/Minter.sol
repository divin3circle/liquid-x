...
contract Minter is CCIPBase {
    ...
    function burn(uint256 _amount) external payable {
        ...
        IERC20(address(dsc)).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
    ...
    function burnAndMint(
        ...
        if (chainSelector == mainRouterChainSelector) {
            IMainRouter(mainRouter).burnAndMint(
                msg.sender,
                chainSelector,
                _amount,
    ...
    function liquidate(
        ...
        if (chainSelector == mainRouterChainSelector) {
            IMainRouter(mainRouter).liquidate(
                chainSelector,
                _liquidatedUser,
                _token,
                _destinationChainSelector,
