...
contract Demo is Test {
    ...
    function setUp() public {
        ...
        mainRouter.setOptimismDepositor(address(depositor));
        mainRouter.setOptimismMinter(address(minter));
        ...
        mainRouter.addAllowedToken(
            chainSelector,
            address(weth),
            address(wethMockAggregator),
            18
        );
        mainRouter.addAllowedToken(
            chainSelector,
            address(wbtc),
            address(wbtcMockAggregator),
            18
        );
    }
    ...
    modifier Deposit(ERC20Mock _token, uint256 _amount) {
        ...
        assert(
            mainRouter.getDeposited(user, chainSelector, address(_token)) ==
                _amount
        );
        _;
    }
    ...
    modifier DepositAndMint(
        ...
        assert(
            mainRouter.calculateHealthFactor(_amountDeposit, _amountMint) > 1
        );
    ...
    function test_deposit() external {
        ...
        assert(
            mainRouter.getDeposited(user, chainSelector, address(weth)) ==
                1000 * 1 ether
        );
    }
    ...
    function test_redeem() external Deposit(weth, 1000 ether) {
        console.log(
            "Balance of Depositor Contract before: ",
            weth.balanceOf(address(depositor))
        );
        ...
        console.log(
            "Balance of Depositor Contract after: ",
            weth.balanceOf(address(depositor))
        );
    ...
    function test_mint() external Deposit(weth, 1 ether) {
        ...
        console.log(
            "Balance of user minted before minting: ",
            dsc.balanceOf(user)
        );
        ...
        console.log(
            "User total collateral in USD: ",
            mainRouter.getUserOverallCollateralValue(user)
        );
        console.log(
            "Balance of user minted after minting: ",
            dsc.balanceOf(user)
        );
        console.log(
            "User health factor: ",
            mainRouter.getUserHealthFactor(user)
        );
    ...
    function test_burn() external DepositAndMint(weth, 1 ether, 1000 ether) {
        ...
        console.log(
            "Balance of user minted after burning: ",
            dsc.balanceOf(user)
        );
        ...
        assert(
            _amountMint - _amountBurn ==
                mainRouter.getMinted(user, chainSelector)
        );
    }

    function testCantRedeemMoreThanDeposited()
        external
        Deposit(weth, 1000 ether)
    {
    ...
    function testAddMoreThanOneCollateral()
        external
        Deposit(weth, 1 ether)
        Deposit(wbtc, 1 ether)
    {
        console.log(
            "Balance of user collateral in USD: ",
            mainRouter.getUserOverallCollateralValue(user)
        );

        assert(
            mainRouter.getUserCollateralValue(
                user,
                chainSelector,
                address(weth)
            ) == 3000 ether
        ...
        assert(
            mainRouter.getUserCollateralOnChainValue(user, chainSelector) ==
                73000 ether
        );
        assert(mainRouter.getUserOverallCollateralValue(user) == 73000 ether);
    }

    function testCanReddemWhenHealthFactorIsMoreThanOne()
        external
        DepositAndMint(weth, 1 ether, 1000 ether)
    {
        uint256 amountRedeem = 0.5 ether;

        console.log(
            "User health factor before redeeming: ",
            mainRouter.getUserHealthFactor(user)
        );
        ...
        console.log(
            "User health factor after redeeming: ",
            mainRouter.getUserHealthFactor(user)
        );
    }

    function testCantRedeemWhenHealthFactorIsLessThanOne()
        external
        DepositAndMint(weth, 1 ether, 1000 ether)
    {
        ...
        console.log(
            "User health factor if redeeming: ",
            mainRouter.calculateHealthFactor(0.8 ether, 1000 ether)
        );
        ...
        DepositAndMint(weth, 1 ether, 1000 ether)
    {
        ...
        assert(
            depositor.getDeposited(liquidator, address(_token)) ==
                _amountDeposit
        );
        assert(
            mainRouter.getDeposited(
                liquidator,
                chainSelector,
                address(_token)
            ) == _amountDeposit
        ...
        console.log(
            "User health factor after liquidating: ",
            mainRouter.getUserHealthFactor(user)
        );
        console.log(weth.balanceOf(liquidator));
    }
