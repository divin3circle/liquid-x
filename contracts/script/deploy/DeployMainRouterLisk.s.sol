...
contract DeployMainRouter is Script, Parameters {
    function run() external returns (MainRouter) {
        ...
        MainRouter mainRouter = new MainRouter(
            CHAIN_SELECTOR_OPTIMISM_SEPOLIA, CCIP_ROUTER_OPTIMISM, OPTIMISM_FUNCTIONS_ROUTER, OPTIMISM_SEPOLIA_DON_ID
        );
        ...
        mainRouter.setAllowedDestinationChain(CHAIN_SELECTOR_AVALANCHE_FUJI, true);
        mainRouter.setAllowedDestinationChain(CHAIN_SELECTOR_ETHEREUM_SEPOLIA, true);
        mainRouter.setAllowedDestinationChain(CHAIN_SELECTOR_POLYGON_AMOY, true);
        mainRouter.setAllowedDestinationChain(CHAIN_SELECTOR_BASE_SEPOLIA, true);
        mainRouter.setAllowedDestinationChain(CHAIN_SELECTOR_OPTIMISM_SEPOLIA, true);
        mainRouter.setAllowedDestinationChain(CHAIN_SELECTOR_ARBITRUM_SEPOLIA, true);
    ...
    function addAllowedToken(MainRouter mainRouter, TokenConfig.Token memory token) public {
        ...
        mainRouter.addAllowedToken(token.ckes.chainSelector, token.ckes.token, token.ckes.priceFeed, 18);
        mainRouter.addAllowedToken(token.gold.chainSelector, token.gold.token, token.gold.priceFeed, 6);
        mainRouter.addAllowedToken(token.usdt.chainSelector, token.usdt.token, token.usdt.priceFeed, 6);
        mainRouter.addAllowedToken(token.op.chainSelector, token.op.token, token.op.priceFeed, 18);
    }
}
